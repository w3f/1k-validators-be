...
export class ChainData {
  ...
  getOpenGovReferenda = async () => {
    ...
    for (const [i, [key, info]] of referenda.entries()) {
  ...
  getConvictionVoting = async () => {
    try {
      ...
      logger.info(`Querying referenda.....`, { label: "Democracy" });
      ...
      logger.info(`Querying conviction voting from the chain...`, {
        label: "Democracy",
      });
      const votingFor =
        await this.api.query.convictionVoting.votingFor.entries();
      logger.info(`Got voting for ${votingFor.length} entries`, {
        label: "Democracy",
      });
      ...
      logger.info(`${casting.length} casting entries`, {
        label: "Democracy",
      });
      logger.info(`${delegating.length} delegating entries`, {
        label: "Democracy",
      });
        ...
        for (const [index, referendumVote] of votes.entries()) {
          ...
          if (isReferendumOngoing) {
            if (voteType["Standard"]) {
              ...
              logger.info(`Vote type is unknown`, { label: "Democracy" });
              logger.info(`Vote type: ${JSON.stringify(voteType)}`, {
                label: "Democracy",
              });
            }
          }
      ...
      logger.info(`Added ${ongoingVotes.length} ongoing casting votes`, {
        label: "Democracy",
      });
      ...
      logger.info(`Added ${allDelegations.length} delegations`, {
        label: "Democracy",
      });
      ...
      logger.info(`Finished querying ongoing delegations`, {
        label: "Democracy",
      });
      ...
      logger.warn(`Error in getConvictionVoting: ${JSON.stringify(e)}`, {
        label: "Democracy",
      });
    }
  };
