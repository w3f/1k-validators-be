 ...
 */
import { ChainData, logger, queries, ScoreKeeper } from "../index";
import { scorekeeperLabel } from "./scorekeeper";
import { jobStatusEmitter } from "../Events";
import { Job, JobStatus } from "./jobs/JobsClass";

export const registerAPIHandler = (chaindata: ChainData, bot: any): void => {
  ...
  chaindata.handler.on(
    "someOffline",
    async (data: { offlineVals: string[] }) => {
  ...
  chaindata.handler.on("newSession", async (data: { sessionIndex: string }) => {
